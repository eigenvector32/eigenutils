...
import { DualMultiArgEmitter } from "../DualMultiArgEmitter";
import { FireMode } from "../FireMode";
import { IDisposable } from "../../IDisposable";

describe('Tests for DualMultiArgEmitter', () => {
    test('Objects held only by the emitter are finalized', async () => {
        expect(gc).toBeDefined();

        let testHandler: (() => void) | null = () => { };

        const emitter: DualMultiArgEmitter<[number, string]> = new DualMultiArgEmitter<[number, string]>();
    ...
    test('Listener which is finalized is not called', async () => {
        ...
        let testHandler: ((newNumber: number, newString: string) => void) | null = (newNumber: number, newString: string) => {
    ...
    test('Listener which is finalized is removed during fire', async () => {
        ...
        let testHandler: ((newNumber: number, newString: string) => void) | null = (_newNumber: number, _newString: string) => {
        };
    ...
    test('Disposing a listener removes the event handler', () => {
        ...
        const emitter: DualMultiArgEmitter<[number, string]> = new DualMultiArgEmitter<[number, string]>(FireMode.Synchronous);
    ...
    test('Fire Microtask', async () => {
        ...
        const emitter: DualMultiArgEmitter<[number, string]> = new DualMultiArgEmitter<[number, string]>(FireMode.Microtask);
        emitter.event((newNumber: number, newString: string) => {
            testNumber = newNumber;
            testString = newString;
        });
    ...
    test('Fire Debounce', async () => {
        ...
        const emitter: DualMultiArgEmitter<[number, string]> = new DualMultiArgEmitter<[number, string]>(FireMode.Debounce);
        emitter.event((newNumber: number, newString: string) => {
            testNumber = newNumber;
            testString = newString;
            count++;
    ...
    test('Fire Microtask | Debounce', async () => {
        ...
        const emitter: DualMultiArgEmitter<[number, string]> = new DualMultiArgEmitter<[number, string]>(FireMode.Microtask | FireMode.Debounce);
        emitter.event((newNumber: number, newString: string) => {
            testNumber = newNumber;
            testString = newString;
            count++;
